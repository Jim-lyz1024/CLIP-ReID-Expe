...
def create_tsne_visualization(features, ids, save_path='hare_tsne_vis.pdf'):
    """Create and save TSNE visualization using seaborn."""
    ...
    df = pd.DataFrame({
        'x': features_2d[:, 0],
        'y': features_2d[:, 1],
        'ID': [f'ID {id_num}' for id_num in ids]
    })
    ...
    # Custom color palette for 31 IDs
    color_palette = [
        '#065143',  # green
        '#00FF00',  # Lime
        '#0000FF',  # Blue
    ...
    # Customize plot
    # plt.title('t-SNE Visualization of Hare ReID Features', 
            #  fontsize=14, pad=20)
